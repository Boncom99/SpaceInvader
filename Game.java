...
public class Game {
	...
	void initialize() {
		...
		ship = new Ship(X, f.HEIGHT / 2, 21, 52, speedShip, shipColor, 3, 2);

		// Aliens position
		initialX = 2 * (f.WIDTH - (AliensWidth + marginH) * numOfAliensPerRow) / 3 + 100;
		initialY = (f.HEIGHT - (AliensHeight + marginV) * numOfAliensPerColumn) / 2 + 15;
		...
		// Wall
		//
		totalMovesVertical = initialY * 2 - 40;
	...
	void GenerateAliens() {
		...
		for (int i = 0; i < numOfAliensPerColumn; i++) {
			for (int j = 0; j < numOfAliensPerRow; j++) {
				aliens.add(new Alien(initialX + j * (AliensWidth + marginH), initialY + i * (AliensHeight + marginV),
						AliensWidth, AliensHeight, speedAliens, aliensColor, AliensLives, totalMovesVertical));
			}
		}
	...
	void aliensShoot() {
		...
		if (time < 0 || time > ShootingRate) {
			...
			if (aliens.size() > 0) {
				...
				bulletsAliens.add(
						new Bullet(al.x + al.width, al.y + ship.height / 2, 8, 4, speedBulletsAlien, (Color.ORANGE)));
			}
		}
	}
